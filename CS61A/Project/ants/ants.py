...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i = 0
        p = self.place
        while (p != None) and i < self.lower_bound:
            p = p.entrance
            i += 1

        while (p != None) and (len(p.bees) == 0) and (not p.is_hive) and i < self.upper_bound:
            p = p.entrance
            i += 1
        ...
        return random_bee(p.bees) # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if amount >= self.health:
            reflect_damage += self.damage
        super().reduce_health(amount)
        for b in list(bs):
            b.reduce_health(reflect_damage)
        # END Problem 5
